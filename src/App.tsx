...
const announceHTML = `<div class="announce">
  ...
  <br/>
  <a href="https://aecheck.tistory.com/36" target="_blank" rel="noreferrer">Patch Note</a>
</div>`

function App() {

  const announceViewed = Boolean(window.localStorage.getItem("a_v_230112"))

  const { lang } = useContext(AnotherContext)
  const message = require(`./language/${lang}.json`)

  useEffect(() => {
    window.localStorage.removeItem("a_v_230111")
    if(!announceViewed) {
    // if(true) {
      Swal.fire({
        title: 'Update 23.01.12',
        html: announceHTML,
        icon: 'info',
      }).then(() => {
        window.localStorage.setItem("a_v_230112", "true")
      })
    }
  }, [])
